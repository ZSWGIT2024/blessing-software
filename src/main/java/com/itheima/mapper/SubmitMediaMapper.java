...
@Mapper
public interface SubmitMediaMapper {
     ...
     * 插入媒体记录
     */
    @Insert("INSERT INTO submit_media (user_id, media_type, filename, original_name, file_path, " +
     ...
     * 根据ID查询
     */
    @Select("SELECT * FROM submit_media WHERE id = #{id}")
     ...
     * 更新状态
     */
    @Update("UPDATE submit_media SET status = #{status}, update_time = NOW() WHERE id = #{id}")
    int updateStatus(@Param("id") Integer id, @Param("status") String status);

    //删除媒体
    @Delete("DELETE FROM submit_media WHERE id = #{id}")
    int delete(Integer id);

    //更新该媒体的点赞状态，0代表false未点赞，1代表true已点赞
    @Update("UPDATE submit_media SET is_liked = #{isLiked} WHERE id = #{id}")
     ...
     * 增加查看次数
     */
    @Update("UPDATE submit_media SET view_count = view_count + 1 WHERE id = #{id}")
    int incrementViewCount(Integer id);

    /**
     * 增加点赞数
     */
    @Update("UPDATE submit_media SET like_count = like_count + 1 WHERE id = #{id}")
    int incrementLikeCount(Integer id);

    /**
     * 减少点赞数
     */
    @Update("UPDATE submit_media SET like_count = GREATEST(like_count - 1, 0) WHERE id = #{id}")
     ...
     * 查询用户最近上传
     */
    @Select("SELECT * FROM submit_media WHERE user_id = #{userId} AND status = 'active' or status = 'pending' " +
