...
public class MediaServiceImpl implements MediaService {
    ...
    @Override
    public PageBean<MediaVO> queryMediaList(MediaQueryDTO queryDTO) {
        // 1. 设置默认值
        ...
        // 3. 查询总记录数
        Long total = mediaMapper.countByCondition(queryDTO);

        // 4. 查询数据列表
        List<UserMedia> mediaList = mediaMapper.selectByCondition(queryDTO);
        ...
        List<MediaVO> voList = mediaList.stream()
                .map(media -> {
                    ...
                    if (queryDTO.getUserId() != null) {
                        Integer likeCount = mediaLikeMapper.countByMediaAndUser(media.getId(), queryDTO.getUserId());
    ...
    @Override
    public PageBean<MediaVO> queryMediaByUserAndStatus(MediaQueryDTO queryDTO) {
        // 1. 设置默认值
        if (queryDTO.getPageNum() == null || queryDTO.getPageNum() < 1) {
            queryDTO.setPageNum(1);
        }
        ...
        List<MediaVO> voList = mediaList.stream()
                .map(media -> {
                    ...
                    // 检查当前用户是否已点赞
                    if (queryDTO.getUserId() != null) {
                        Integer likeCount = mediaLikeMapper.countByMediaAndUser(
    ...
    @Override
    public PageBean<MediaVO> queryAllStatusMediaByUser(MediaQueryDTO queryDTO) {
        // 1. 设置默认值
        if (queryDTO.getPageNum() == null || queryDTO.getPageNum() < 1) {
            queryDTO.setPageNum(1);
        }
    ...
    public MediaVO getMediaDetail(Integer mediaId, Integer userId) {
        ...
        // 2. 如果不是公开的，检查权限
        if (!media.getIsPublic() && (userId == null || !userId.equals(media.getUserId()))) {
            throw new RuntimeException("没有权限查看此文件");
        }

        // 3. 增加查看次数
        mediaMapper.incrementViewCount(mediaId);

        // 4. 检查当前用户是否已点赞
        boolean liked = false;
        if (userId != null) {
            Integer likeCount = mediaLikeMapper.countByMediaAndUser(mediaId, userId);
            liked = likeCount > 0;
        }
    ...
    public MediaVO updateMedia(Integer mediaId, MediaUpdateDTO updateDTO) {
        ...
        // 4. 执行更新
        int rows = mediaMapper.update(updateMedia);
        if (rows == 0) {
    ...
    public boolean toggleLike(Integer mediaId, Integer userId) {
        try {
            ...
            // 3. 检查是否已经点赞
            MediaLike existingLike = mediaLikeMapper.selectByMediaAndUser(mediaId, userId);

            if (existingLike != null) {
                // 已经点赞，执行取消点赞
                mediaLikeMapper.delete(existingLike.getId());
                // 更新点赞数
                mediaMapper.decrementLikeCount(mediaId);
                // 更新点赞状态
                mediaMapper.updateLikeStatus(mediaId, 0);
                ...
                mediaLikeMapper.insert(newLike);
                // 更新点赞数
                mediaMapper.incrementLikeCount(mediaId);
                //更新点赞状态
                mediaMapper.updateLikeStatus(mediaId, 1);
    ...
    public boolean updateStatus(Integer id, String status) {
        try {
            UserMedia media = mediaMapper.selectById(id);
            if (media == null) {
                throw new RuntimeException("媒体文件不存在");
            }
            int rows = mediaMapper.updateStatus(id, status);
    ...
    public boolean batchUpdateStatus(List<Integer> ids, String status, String tags) {
        ...
        // 更新数据库
        int affectedRows = mediaMapper.batchUpdateStatus(ids, status, tags);
    ...
    public List<UserMedia> getRecentMedia(Integer userId, Integer limit) {
        ...
        return mediaMapper.selectRecentByUserId(userId, limit);
    }
...
//            userMedia.setThumbnailPath(thumbnailPath);
            ...
            try {
                int result = mediaMapper.insert(userMedia);
